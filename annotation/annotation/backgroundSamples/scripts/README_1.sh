...
ls $QUANT/*.fastq.gz | perl -pe "s#$QUANT/##" > $QUANT_ALIGN/sampleInfo1.txt
##### adding this to subset from the sample infor file
while read pat; do grep -w "^$pat" $QUANT_ALIGN/sampleInfo1.txt; done < /groups/ameres/Pooja/Projects/zebrafishAnnotation/sequencingRun_december2017/analysis/annotation/backgroundSamples/data/sampleInfo_uninjected.txt  > $QUANT_ALIGN/sampleInfo.txt
